    ...
    for q_level in [2, 1]:
        for hs_per_node in [4]:
            for topology in Path(f'{LCDN_PATH}Topologies/TZ_small').iterdir():
                for i in range(5):
                    new_paras = TestParameters(1, 0, 0, SEEDS[i], topology, i, hs_per_node, 50, q_level, RerouteStrategy.QCHANGES)
...
def run_all_tests():
    ...
    with open(f'{LCDN_PATH}Evaluations/LCDN/Paper/Flow2/q_level_result.json', 'w') as file:

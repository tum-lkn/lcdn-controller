...
def create_all_tasks():
    ...
    for init_ksp in [1, 2, 3, 4]:
        for hs_per_node in [1, 2, 3, 4]:
            for topology in Path(f'{LCDN_PATH}Topologies/TZ_small').iterdir():
                for i in range(5):
                    new_paras = TestParameters(init_ksp, 0, 0, SEEDS[i], topology, i, hs_per_node, 50, 0)
...
def run_all_tests():
    ...
    for (parameter, flow_data) in result:
        if f'hpn{parameter.hosts_per_node}' not in result_dict[f'initksp{parameter.init_ksp}'].keys():
            result_dict[f'initksp{parameter.init_ksp}'][f'hpn{parameter.hosts_per_node}'] = {}

        if parameter.topology_path.name not in result_dict[f'initksp{parameter.init_ksp}'][f'hpn{parameter.hosts_per_node}'].keys():
            result_dict[f'initksp{parameter.init_ksp}'][f'hpn{parameter.hosts_per_node}'][parameter.topology_path.name] = {}
        
        result_dict[f'initksp{parameter.init_ksp}'][f'hpn{parameter.hosts_per_node}'][parameter.topology_path.name][parameter.run] = flow_data
        
    with open(f'{LCDN_PATH}Evaluations/LCDN/Paper/Flow1/init_ksp_results.json', 'w') as file:

...
def create_all_tasks():
    ...
    for ksp_offset in [1, 2, 3]:
        for hs_per_node in [1, 2, 3, 4]:
            for topology in Path(f'{LCDN_PATH}Topologies/TZ_small').iterdir():
                for i in range(5):
                    new_paras = TestParameters(1, 0, ksp_offset, SEEDS[i], topology, i, hs_per_node, 50, 0)
...
def run_all_tests():
    ...
    for (parameter, flow_data) in result:
        if f'hpn{parameter.hosts_per_node}' not in result_dict[f'offset{parameter.ksp_offset}'].keys():
            result_dict[f'offset{parameter.ksp_offset}'][f'hpn{parameter.hosts_per_node}'] = {}

        if parameter.topology_path.name not in result_dict[f'offset{parameter.ksp_offset}'][f'hpn{parameter.hosts_per_node}'].keys():
            result_dict[f'offset{parameter.ksp_offset}'][f'hpn{parameter.hosts_per_node}'][parameter.topology_path.name] = {}
        
        result_dict[f'offset{parameter.ksp_offset}'][f'hpn{parameter.hosts_per_node}'][parameter.topology_path.name][parameter.run] = flow_data
        
    with open(f'{LCDN_PATH}Evaluations/LCDN/Paper/Flow1/ksp_offset_results.json', 'w') as file:

    ...
    for init_ksp in [1, 4]:
        for hs_per_node in [ 4]:
            for topology in Path(f'{LCDN_PATH}Topologies/TZ_small').iterdir():
                for i in range(5):
                    new_paras = TestParameters(init_ksp, 0, 0, SEEDS[i], topology, i, hs_per_node, 50, 0, RerouteStrategy.QCHANGES)
...
def run_all_tests():
    ...
    with open(f'{LCDN_PATH}Evaluations/LCDN/Paper/Flow2/init_ksp_results.json', 'w') as file:

...
from Routing.routing import LCDNStrategy
...
def create_all_tasks():
    ...
    for topology in Path(f'{LCDN_PATH_LKN}Topologies/TZ_small').iterdir():
        for p in probs:
            for i in range(5):
                new_paras = Settings(0, SEEDS[i], topology, i, 4, 50, LCDNStrategy.GREEDYMIX, p)
...
def run_all_tests():
    ...
    for (parameter, flow_data) in result:
        ...
        if parameter.greedy_p not in result_dict[parameter.topology_path.name]:
            result_dict[parameter.topology_path.name][parameter.greedy_p] = {}

        result_dict[parameter.topology_path.name][parameter.greedy_p][parameter.run] = flow_data
        
    with open(f'{LCDN_PATH_LKN}Evaluations/LCDN/Paper/Flow2/mixgreedy_hpn4.json', 'w') as file:
